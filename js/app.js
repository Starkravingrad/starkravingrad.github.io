...

                {
                    ...
                    children: [
                        ...
                        },
                        {
                            ...
                            link: "http://alpine-adventures.firebaseapp.com"
                        }]
                    ...
                    children: [
                        ...
                        {
                            name: 'This Is Where Im From',
                            image: "/img/thisiswhereimfrom.png",
                            link: "http://www.thisiswhereimfrom.com"
                        },
                        {
                            name: 'Danktronics',
...
hammertime.on('pan', function(ev) {
    ...
    mouse.y = ev.deltaY;
                ...
                var geometry = new THREE.SphereGeometry(1000, 100, 1, 5);
                var material = new THREE.MeshBasicMaterial({ color: 0x00ff00 });
                cube = new THREE.Mesh(geometry, planetMaterial);
            ...
            function detectCollisions() {
                ...
                for (var i = 0; i < enemies.length; i++) {
                    var enemyVector = new THREE.Vector3(enemies[i].position.x, enemies[i].position.y, enemies[i].position.z);

                    var enemyDistance = enemies[i].position.distanceTo(playerPosition);
                    var hypotenuse = enemyDistance;
                    var subVector = new THREE.Vector3();
                    subVector = subVector.subVectors(enemyVector, playerVector);



                    //  var enemyIntersections = enemyCaster.intersectObjects(objects);
                    //    enemyCaster.ray.origin.copy(enemyVector);
                    //  
                    //  console.log(enemyIntersections)
                    //  
                    //    var isEnemyOnObject = enemyIntersections.length > 0;
                    //    
                    //    if(isEnemyOnObject === true) {
                    //        scene.remove(enemies[i]);
                    //        createEnemy();
                    //    }

                    if (hypotenuse > 3.5) {
                        enemies[i].position.y -= 1 * (subVector.y / hypotenuse);

                        enemies[i].position.x -= 1 * (subVector.x / hypotenuse);
                        enemies[i].position.z -= 1 * (subVector.z / hypotenuse);
                    } else {
                        scene.remove(enemies[i]);
            ...
            function animate() {
                ...
                //console.log(group1.children[0].geometry.vertices[0]);
                //console.log(mouse);
                detectCollisions();
                // find intersections
                //console.log(contentBox);
                ...
                var spring_x = k * ((group1.rotation.y - mouse.y));
                ...
                group1.position.x += group1.vx * 0.01;
                group1.position.y += group1.vx * 0.01;
                group1.position.z += group1.vx * 0.01;
                ...
                group1.rotation.x += group1.vx * 0.01;
                group1.rotation.y += group1.vx * 0.01;

                group1.rotation.z += group1.vx * 0.01;
